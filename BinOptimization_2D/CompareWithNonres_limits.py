...
for ch in channels:
	...
	ratio.GetYaxis().SetTitle("#frac{New binnings}{Old binnings}")
	...
	k=1
		...
		fnew1='{0}_{1}/BestBin.txt'.format(ch,sel)
		fnew2='backup/{0}_{1}/BestBin.txt'.format(ch,sel)
	...
	#		nbins_new=new1.nbins
		...
		fold=rt.TFile('/data_CMS/cms/liugeliang/HHbbtautau_histos/Legacy2018_{0}_25Mar2021/analyzedOutPlotter.root'.format(ch),'read')
		if '1b1j' in sel:
			j=0
		elif '2b0j' in sel:
			j=1
			...
			j=3
		print "data_obs_{0}_SR_DNNoutSM_kl_1".format(categories[j])
		template=fold.Get("data_obs_{0}_SR_DNNoutSM_kl_1".format(categories[j])).Clone()
		nbins_old=template.GetNbinsX()
		fold.Close()
		
		h_new.SetBinContent(k,limit_new1)
		h_old.SetBinContent(k,limit_new2)
		ratio.SetBinContent(k,1.*limit_new1/limit_new2)
		if 'mHH' in sel:
			m='mHH'+sel.split('mHH')[1]
		else:
			m='InvDR'
        ...
        legend.AddEntry(h_new,'New binnings','P')
        legend.AddEntry(h_old,'Old binnings','P')
	...
	c1.SaveAs("Output/Comparison_limits_{0}.pdf".format(ch),"pdf")
