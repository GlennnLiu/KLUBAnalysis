...
pt2.SetBorderSize(0)
pt2.SetFillColor(0)
pt2.SetTextSize(0.047*3)
pt2.SetTextFont(42)
pt2.SetFillStyle(0)
pt2.AddText("2018 - 59.7 fb^{-1} (13 TeV)")

c1 = rt.TCanvas("c1", "c1", 1000,1000)#650, 500)
...
hframe.GetYaxis().SetTitleSize(0.047*2)
hframe.GetXaxis().SetTitleSize(0.055*2)
hframe.GetYaxis().SetLabelSize(0.045*2)
hframe.GetXaxis().SetLabelSize(0.055*2)
...
for ch in channels:
	...
	k=1
		...
		for m in mHH:
			...
			fold1='/data_CMS/cms/liugeliang/HHbbtautau_BinOpt/Kons/backup9/{0}_{1}{2}/BestBin.txt'.format(ch,ca,m)
			fnew2='/data_CMS/cms/liugeliang/HHbbtautau_BinOpt/Kons/backup8/{0}_{1}{2}/BestBin.txt'.format(ch,ca,m)
			fnew1='/data_CMS/cms/liugeliang/HHbbtautau_BinOpt/Kons/backup10/{0}_{1}{2}/BestBin.txt'.format(ch,ca,m)
			...
			old1=Binning2(fold1)
                        old2=Binning2(fold2)
			if new1.target>new2.target:
				nbins_new=new1.nbins
			else:
				nbins_new=new2.nbins
			if old1.target>old2.target:
				nbins_old=old1.nbins
			else:
				nbins_old=old2.nbins
			...
			h_old.SetBinContent(k,nbins_old)
			ratio.SetBinContent(k,1.*nbins_new/nbins_old)
			hframe.GetXaxis().SetBinLabel(k,names[j]+"_"+m)
			k=k+1
        Hnew.append(h_new)
...
pad0=rt.TPad("pad1","pad1",0,0.95,1,1,-1,0,0)
pad0.SetTopMargin(0.0)
pad0.SetBottomMargin(0.0)
pad0.Draw()

pad1=rt.TPad("pad1","pad1",0,0.7,1,0.95,-1,0,0)
...
pad2=rt.TPad("pad1","pad1",0,0.45,1,0.7,-1,0,0)
...
pad3=rt.TPad("pad1","pad1",0,0.20,1,0.45,-1,0,0)
...
pad4=rt.TPad("pad1","pad1",0,0,1,0.2,-1,0,0)
pad4.SetTopMargin(0.)
pad4.SetBottomMargin(0.99)
pad4.Draw()
...
c1.SaveAs("Comparison_nbins.png")
