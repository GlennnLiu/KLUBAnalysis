...
class Binning:
	...
	def Protection(self,tot_bkg_y,ref_bkg_y):
...
class BayesianOptimization:
	def __init__(self,work_area,old_input,acq,kappa,xi):
	...
	def RegisterOld(self,old_input):
		for stra in ['ConstSize','FlatB','FlatS','FlatSB']:
			for nbins in range(max_nbins):
				if os.path.exists("{0}/{1}_{2}".format(old_input,stra,str(nbins))):
					...
					prot=binning.Protection(self.tot_bkg_y,self.ref_bkg_y)
					if not prot:
						continue
	...
	def UpdateBounds(self,f):
		...
		#old_bounds=target_space.bounds
		...
		f.write("nbins of best binning: "+str(nPoints)+"\n")
		#for i in range(max_nbins):
...
parent='/home/llr/cms/liugeliang/HH_bbtautau/KLUBAnalysis/BinOptimization_Kons/'
path=parent+ch+"_"+ca
if not os.path.exists(path):
        os.system("mkdir "+path)

old_input='/home/llr/cms/liugeliang/HH_bbtautau/KLUBAnalysis/BinOptimization_Jona/{0}_{1}'.format(ch,ca)
...
for nbins in range(6,50):
	work_area="{0}/nbins{1}".format(path,str(nbins))
	os.system("mkdir "+work_area)
	ob=BayesianOptimization(nbins,work_area,old_input,'ucb',2.57,0.1)
	ob.Write()
	if BestBinning == None:
		BestBinning=ob.BestBinning
	elif not BestBinning.isBetter(ob.BestBinning):
		BestBinning=ob.BestBinning
		stop_point=0
	else:
		stop_point+=1
...
'''
os.system("mkdir "+path)
ob=BayesianOptimization(path,old_input,'ucb',2.57,0.1)
ob.Write()
